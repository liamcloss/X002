...
def normalise_universe(
    ...
    for instrument in instruments:
        ...
        row['active'] = _infer_active(instrument)
        rows.append(row)
...
def _infer_active(instrument: dict[str, Any]) -> bool:
    max_open_quantity = instrument.get('maxOpenQuantity')
    if max_open_quantity is None:
        return True
    try:
        return float(max_open_quantity) > 0
